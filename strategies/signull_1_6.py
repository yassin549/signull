"""
Signull 1.6 — Option SIM 1.1 Model Probability Strategy.

Uses the Option SIM 1.1 model (Direction TCN SIM 1.1) to estimate the probability
of candle outcome (P(Up) and P(Down)) every second from BTC 1s data.

Enters whichever option side (UP or DOWN) first reaches a target probability of success
(default 65%, dynamically configurable via strategy parameters).
fill uses executable market mid/ask price at the signal tick.
"""
...
STRATEGY_CLASS = "Signull16Strategy"

logger = logging.getLogger(__name__)


def _as_unit_interval(value: float, name: str) -> float:
    """Accept 0.65 or 65 for percentage-like params."""
    v = float(value)
    if v > 1.0:
        v = v / 100.0
    if not (0.0 < v < 1.0):
...
class Signull16Strategy(Strategy):
    """
    Signull 1.6: Option SIM 1.1 model probability strategy.

    Buys whichever side (UP or DOWN) reaches the target probability (default 65%,
    configurable dynamically via 'threshold' parameter) first in Option SIM 1.1.
    """

    meta = StrategyMeta(
        id="signull_1_6",
        name="Signull 1.6 (Option SIM 1.1 65% Target)",
        description=(
            "Buys whichever side (UP or DOWN) first reaches a target win probability "
            "(default 65%, dynamic strategy parameter) as calculated by the Option SIM 1.1 "
            "causal direction TCN model. Fill uses the market mid at the signal tick. "
            "Risks a configurable fixed fraction of capital (default 10%)."
        ),
        default_params={
            "threshold": 0.65,
            "risk_pct": 0.10,
    ...
    def __init__(self, params: dict[str, Any] | None = None):
        ...
        try:
            self.params["threshold"] = _as_unit_interval(
                self.params.get("threshold", 0.65), "threshold"
            )
        ...
        try:
            self.params["risk_pct"] = _as_unit_interval(
                self.params.get("risk_pct", 0.10), "risk_pct"
            )
        except (TypeError, ValueError):
    ...
    def prepare_backtest(self, candles, progress_callback=None) -> None:
        """
        Score every candle in the run with the Option SIM 1.1 direction model.

        Downloads BTC 1s history as needed, builds causal features, runs the
        network — actual Option SIM 1.1 model inference.
        """
        if not candles:
            raise ValueError("No candles supplied to signull_1_6")
        ...
        try:
            ...
            raise ValueError(
                f"Option SIM 1.1 model inference failed: {type(exc).__name__}: {exc}"
            ) from exc
        ...
        logger.info(
            "signull_1_6 ready: coverage %s/%s (%.1f%%) source=%s",
        ...
        if cov["candles_with_probs"] <= 0:
            raise ValueError(
                "Option SIM 1.1 produced probabilities for 0 of "
                f"{cov['candles_requested']} backtest candles. "
                "Binance 1s data may be missing for this range, "
    ...
    def _get_store(self):
        if self._store is None:
            raise RuntimeError(
                "Option SIM 1.1 direction store not built. prepare_backtest() must run first."
            )
        return self._store
    ...
    def evaluate(
        ...
        *,
        ...
        try:
            threshold = _as_unit_interval(self.params.get("threshold", 0.65), "threshold")
        except (TypeError, ValueError):
            threshold = 0.65
        ...
        return TradeSignal(
            ...
            price=market_px,
            reason=(
                f"Option SIM 1.1 {side.upper()} {model_p:.1%} ≥ {threshold:.0%} "
                f"@ t+{cross_sec}s -> buy {side.upper()} @ {market_px:.0%}"
            ),
    ...
    def position_risk_fraction(
        ...
        try:
            risk_fraction = _as_unit_interval(self.params.get("risk_pct", 0.10), "risk_pct")
        except (TypeError, ValueError):
            risk_fraction = 0.10
    ...
    def size_label(self, risk_frac: float) -> str:
        del risk_frac
        try:
            thr = _as_unit_interval(self.params.get("threshold", 0.65), "threshold")
        except (TypeError, ValueError):
            thr = 0.65
        return f"sim1.1≥{thr:.0%}"
